...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below.
    ...
    The `tidyverse` Package is a collection of different R packages and libraries for efficient data handling and visualization with R. Among others, it includes dplyr, ggplot2, readr, tidyr, and tibble.

```{r}
library(tidyverse)
library(ggthemes) #colorblind friendly colors
```
...
1.  Starting with the `waste_data_long_mean` object, execute the code and observe the output in the Console. Would you publish this table in this format in a report?
    ...
    No, it's not very user friendly and also hides most rows.
...
2.  Execute the code and observe the output in the Console. Would you publish this table in a report in this format?
    ...
    It's a little better, but still not good to publish and also still hides variables.
...
ggplot(data = waste_data_long_mean,
  ...
  labs(title = "Waste Composition",
			...
			caption = "Reproduced Plot. Data from: https://datacatalog.worldbank.org/search/dataset/0039597") +
  theme_minimal() +
